...
def str_2xtim(x,ksgn='year'):
    t1=arrow.get(x)
    if (ksgn=='year')or(ksgn=='y'):tss=t1.year
    elif (ksgn=='month')or(ksgn=='m'):tss=t1.month
    elif (ksgn=='day')or(ksgn=='d'):tss=t1.day
    elif (ksgn=='week')or(ksgn=='w'):tss=t1.weekday()
    else :tss=''
...
def sgn_4lst(ksgn,vlst):
    
    xlst=list(map(lambda x:ksgn+'_'+str(x), vlst))
    return xlst
